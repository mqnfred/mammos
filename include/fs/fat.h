...
struct fat_sb_info {
    uint32_t fat; /* The FAT region */
    uint32_t root; /* The root dir region */
    uint32_t data; /* The data region */

    uint32_t clusters; /* Total number of clusters */
    uint32_t root_cluster; /* Cluster number for (FAT32) root dir */
    int32_t root_size; /* The root dir size in sectors */

    int32_t clust_shift; /* based on sectors */
    int32_t clust_byte_shift; /* based on bytes   */
    int32_t clust_mask; /* sectors per cluster mask */
    int32_t clust_size;
...
struct fat_pvt_inode {
    ...
    uint32_t start;     /* Starting sector */
    uint32_t offset;        /* Current sector offset */
    uint32_t here;      /* Sector corresponding to offset */
};
